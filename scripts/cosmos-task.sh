...
wait_for_rpi_rp2_mount() {
  local side="$1"
  local dev_path=""
  echo "please put ${side} hand side into boot mode now"
  echo "IMPORTANT: connect USB directly to the ${side} half while flashing (do not leave halves connected through master/slave)"
  until [[ -n "$dev_path" ]]; do
  ...
  if ! findmnt -rn -S "$dev_path" >/dev/null 2>&1; then
    if ! udisksctl mount -b "$dev_path" >/dev/null 2>&1; then
      echo "auto-mount failed for $dev_path. confirm USB is connected directly to this half, open it in nautilus, then press Enter"
      read -r _
    fi
  fi
}
...
wait_for_rpi_rp2_disconnect() {
  ...
  if [[ -n "$(find_rpi_rp2_device)" ]]; then
    echo "still seeing a boot drive. unplug current half, connect USB directly to the other half in boot mode, then press Enter"
    read -r _
  fi
}
